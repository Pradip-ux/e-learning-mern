...
function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // In login.jsx handleLogin function
      ...
      // If you need the token for logic, extract it like this:
      ...
      <div className="w-full max-w-5xl h-full max-h-[700px] bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] flex overflow-hidden border border-gray-100">
        ...
        <div className="flex flex-col justify-center w-full md:w-1/2 px-8 lg:px-16 py-6">
          <div className="mb-8">
            ...
            <p className="text-gray-400 text-sm font-medium italic">Continue your learning journey</p>
          </div>

          <div className="space-y-5">
            <div className="space-y-1">
              <label className="text-[10px] uppercase tracking-[0.15em] font-bold text-gray-400 ml-1">Email Address</label>
              <input
                type="email"
                className="w-full p-3.5 rounded-xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all outline-none text-sm"
            ...
            <button
              onClick={handleLogin}
              disabled={loading}
              className="w-full bg-gray-900 text-white p-4 rounded-xl font-bold hover:bg-indigo-600 transition-all transform active:scale-95 flex justify-center mt-2 shadow-lg shadow-gray-200"
            >
          ...
          <button
            onClick={googleLogin}
            className="flex items-center justify-center gap-3 border border-gray-200 p-3.5 rounded-xl hover:bg-gray-50 transition-all font-semibold text-sm text-gray-600"
          >
          ...
          <p className="text-center text-gray-400 text-xs mt-8">
            New to SkillNest? <span onClick={() => navigate("/SignUp")} className="text-indigo-600 font-bold cursor-pointer hover:underline underline-offset-4">Create Account</span>
          </p>
        </div>

        {/* RIGHT SIDE: PREMIUM BRANDING (Matching SignUp) */}
        <div className="hidden md:flex w-1/2 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-indigo-600 via-indigo-700 to-purple-900 items-center justify-center flex-col p-12 relative">
          ...
          <div className="absolute bottom-10 right-10 w-40 h-40 bg-purple-500/20 blur-[80px] rounded-full"></div>
          ...
          <div className="text-center mt-10 z-10">
            ...
            <p className="text-indigo-500 text-sm font-medium italic max-w-xs mx-auto leading-relaxed">
              "Unlock your potential with the world's most intuitive learning platform."
            </p>
          </div>
